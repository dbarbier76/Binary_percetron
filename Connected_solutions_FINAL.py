...
def Quenched_theory(N,κ_new,κ_old,α):
    ...
    for j in range(Ntot):
        ...
        if j%max(1,int(Ntot/100))==0:
            ...
            print('Completion:',round(100*j/Ntot,2),'%   mag=',str(m**(j+1)))
...
def Quenched_theory_fixed_m(N,m,κ_old,α):
    κ_min=0.3
    κ_max=5
    N_loop=12
    
    def Quenched_theory_cut(N,m_cut,κ_new,κ_old,κ_min,κ_max,α):
        ### initialization of the distribution
        N_x=300
        N_x_cut=120
        cut=0.95*κ_old
        ...
        m_spin_flip=1-2/N
        Ntot=int(log_(m_cut-0.05)/log_(m_spin_flip))
        pot=10
        
        for j in range(Ntot):
            
            if j%max(1,int(Ntot/1000))==0:
                print("Quench theory, Size:",N)
                print("kappa_min/max:",κ_min,κ_max)
                print("kappa=",str(κ_new))
                print('Completion:',round(100*j/Ntot,2),'%   mag=',str(m_spin_flip**(j+1)),'   pot=',str(pot))
                print("")
            ...
            pot= potential(κ_new,κ_old,α,m_spin_flip,P_x,P_y)
            if pot>0:
                cut=max(κ_new/2,κ_new-10*sqt_(2*(1-m_spin_flip*m_spin_flip)))
                P_x,P_y=distribution_update_2(κ_new,κ_old,α,m_spin_flip,P_x,P_y,cut,N_x,N_x_cut)
    ...
    for k in range(N_loop):
        κ_test=(κ_max+κ_min)/2
        m_test=Quenched_theory_cut(N,m,κ_test,κ_old,κ_min,κ_max,α)
        ...
        if m_test<m:
            ...
            print('freezeing too close from planted signal')
            print("")
            print("")
            print("")
...
#### Adiabatic proc. ####
def Adiabatic_theory(N,κ_old,α):
    ### initialization of the distribution
    N_x=80
    N_x_cut=40
    cut=0.95*κ_old
    ...
    for k in range(Ntot):
        ...
        if k%max(1,1)==0:
                print("Adiabatic theory, Size,m:",N,m)
                print("kappa=",str(κ_old))
                print('Completion:',round(100*k/Ntot,2),'%')
                if k>1:
                    print('breaking:',breaking_loop,dκ_guess*(Ntot-k),abs(kappa_list[0]-kappa_list[1])*10**-4)
        ...
        if breaking_loop<10:
            dκ=optimal_kappa_potential(0.9*dκ_guess,κ_old,α,m,P_x,P_y)
            dκ_guess=dκ
            κ_new=dκ+κ_old
            print("dκ:",dκ)
            print("")
            ...
            if k>1 and dκ_guess*(Ntot-k)<abs(kappa_list[0]-kappa_list[1])*10**-4:
                breaking_loop+=1
            if k>1 and dκ_guess<10**-9:
                breaking_loop+=1                
                
            cut=max(κ_old/2,κ_old-5*sqt_(2*(1-m*m)))
            P_x,P_y=distribution_update_2_simple(κ_new,κ_old,α,m,P_x,P_y,cut,N_x,N_x_cut)

            κ_old=κ_new
            
        else: 
            kappa_list[k+1:Ntot]=κ_old
            break
        
    return kappa_list,m_list
...
def potential(κ_new,κ_old,α,m,P_x,P_y):
    ...
    energetic_term=2*(α/N_w)*integrate.quad(func,-κ_old,-cut )[0]
        
    return entropic_term+energetic_term

def potential_simple(κ_new,κ_old,α,m,P_x,P_y):
    ...
    a=func(P_x)
    b=a+np.roll(a,1)
    Δx=(P_x-np.roll(P_x,1))
    out=b*Δx/2
    out[0]=0

    energetic_term_simple=2*(α/N_w)*np.sum(out)
        
    return entropic_term+energetic_term_simple


#### Solver for Adiabatic theory ####
def optimal_kappa_potential(dκ,κ_old,α,m,P_x,P_y):
    
    def opening_gap(dκ):
        κ_new=κ_old+abs(dκ)
        out= (abs(potential_simple(κ_new,κ_old,α,m,P_x,P_y))**(1/10))
        return out
...
def distribution_update_2_simple(κ_new,κ_old,α,m,P_x,P_y,cut,N_x,N_x_cut):
    ...
    for k in range(N_x):
        ...
        if k<N_x_cut:
            ...
            x_min=max(-10,v1-0.001)
            x_max=min(vo+0.001,10) 
            x_maximum=max(min(-P_x_new[k],x_max),x_min)
            ...
            if print_==1:
                ...
                plt.plot(x_list,y_list,label='true')
                plt.plot([x_maximum,x_maximum],[0,max(y_list)])
                plt.xlabel(str(P_x_new[k]))
                plt.legend()
                plt.show()    
                       


            if abs(func_P(x_min))<(10**(-3.5))*abs(func_P(x_maximum)) and abs(func_P(x_max))<(10**(-3.5))*abs(func_P(x_maximum)):
                P_y_new[k]=integrate.quad(func_P,x_min ,x_max )[0]
            else:
                P_y_new[k]= np.interp(abs(m*P_x_new[k] + sqt_(1-m*m)*0) ,P_x,P_y)/H(κ_new,   m*m*P_x_new[k]+m*sqt_(1-m*m)*0,   1-m*m) 
            ...
            if print_==1:
                ...
                plt.plot([x_max,x_max],[0,max(y_list)])
                plt.plot(x_list,y_list,label='true')
                plt.plot(x_list,y2_list,label='test2')
                plt.plot(x_list,y3_list,label='test3')
                plt.plot([x_maximum,x_maximum],[0,max(y_list)])
                plt.xlabel(str(P_x_new[k]))
                plt.legend()
                plt.show()    
...
def Monte_Carlo(α,κ_old_list,κ_max,N_list,write,ticket):
    ...
    for k in len(N_list):
        ...
        N_flip=9*N      #Number of trial flips per dκ
        ...
        κ=κ_old+0 # Important to keep the +0 !!
        dκ_tot=0
    for  k in range(Nκ):
        κ+=dκ
        dκ_tot+=dκ
...
def Monte_Carlo_2(α,κ_old_list,κ_new_list,N_list,mo_list,write,ticket):
    ...
    ####################################
    N_mem=1500                                   #Number of configuration stored
    ...
    for k in range(N_try):
        ...
        dflip_mem=int(N)   # We store a new config. each time the system has flipped "dflip_mem" spins 
        flip_count=0
        ...
        correlation_list_theory[0]=1
        indice_mem=1
        
        time_accepted_flip=0
        ######################################## 
        ########################################   
        ...
        for j in range(N_flip):
            ...
            if flip_count>dflip_mem and indice_mem<N_mem:
                ...
                time_list_mem[k][indice_mem]=time_accepted_flip
                #########################################
                ...
                ###### Computing the correlations ######
                correlation_list_theory[indice_mem]=(1-2/N)**time_accepted_flip
                ########################################
            ...
            ##### Select an indice #####
            indice_flip=int(np.random.uniform(low=0, high=N, size=1))
            sign_flip=-x[indice_flip]
        ...
        if ticket>0:
            ...
            m_fin=0.01
            index=int(log_(m_fin)/log_(1-2/N))
        ...
        if ticket>5:
            ...
            for j in range(indice_mem):
                ...
                if int(j%20)==1 and plot_index<=N_plot:
                    ...
                    for l in range(min(indice_mem-j,index)):
                        ...
                        if plot_index==1:
                            correlation_th_new[l]=(1-2/N)**time_list_new[l]
                        
                    plt.plot(time_list_new,correlation_new,label="j'="+str(int(time_list_mem[k][j])))
                    if plot_index==1:
                        plt.plot(time_list_new,correlation_th_new,c='black',linestyle='--',label='theoretical prediction')
            ...
            for j in range(indice_mem):
                ...
                if int(j%20)==10 and plot_index<=N_plot:
                    plot_index+=1
...
def Monte_Carlo_22(α,κ_old_list,κ_new_list,N_list,mo_list,write,ticket):
    N_try=len(κ_new_list)
    ...
    flip_memory_cost=     lambda index_flip,prior_index_flip: 2*min(  1,  np.min(abs(index_flip*np.ones(len(prior_index_flip))-prior_index_flip))  )  -1
    accept_flip_backward= lambda index_flip,prior_index_flip,β: np.sign(    e_(β* flip_memory_cost(index_flip,prior_index_flip)  )/(2*cosh_(β))      -   np.random.uniform(0,1,1)  )       
    ...
    ### magnetization  of the system ###
    ####################################
    N_mem=3000                                    #Number of configuration stored
    mag_list_time=np.zeros((N_try,N_mem))        #Store the overlap with the planted signal in rescaled time steps
    time_list_mem=np.zeros((N_try,N_mem))        #Store the true time steps of the quench
    time_cut=np.zeros(N_try)
    ####################################
    ...
    for k in range(N_try):
        ...
        β_eff=50          #Effective temp that prevents from doing two times the same spin flip
        M=int(α*N)        #Number of interactions
        
        N_flip=int(10350*N)  #Number of trial flips per try
        ...
        N_prior=10
        prior_indice_flip=-3*np.ones(N_prior)
        ####################################
        ####################################
        ...
        dflip_mem=int(N/200)   # We store a new config. each time the system has flipped "dflip_mem" spins 
        flip_count=0
        ...
        for j in range(N_flip):
            ...
            if flip_count>dflip_mem and indice_mem<N_mem:
                ...
                ###### Computing the correlations ######
                correlation_list_theory[indice_mem]=(1-2/N)**(dflip_mem*indice_mem)
                ########################################
            ...
            ##### Select an indice #####
            indice_flip=int(np.random.uniform(low=0, high=N, size=1))
            #indice_flip=indice_flip%N
            sign_flip=-x[indice_flip]
            
            
            ##### Flipping rule #####
            if flip_break==0 :#and accept_flip_backward(indice_flip,prior_indice_flip,β_eff)>0:
                ...
                if perceptron_sum_dif(interaction_vector,G_transpose,indice_flip,sign_flip,κ)<1/M:
                    ...
                    prior_indice_flip=np.roll(prior_indice_flip,1)
                    prior_indice_flip[0]=indice_flip
        ...
        if ticket>0:
            ...
            ### printing overlap with the planted conf. ###
            m_fin=0.01
            index=int((log_(m_fin)/log_(1-2/N))/dflip_mem)
            ...
            plt.plot(log_(mag_list_time[k][0:min(index,indice_mem)]))
            plt.plot(log_(correlation_list_theory[0:min(index,indice_mem)]))
            plt.title(r'$\alpha=$'+str(α)+'   '+r'$\kappa_{initial}=$'+str(κ_old))
            plt.ylabel(r'$m_{j,0}$')
            plt.xlabel(r'$j$')
            plt.show()
        ...
        if ticket>5:
            ...
            for j in range(indice_mem):
                ...
                if int(j%20)==1 and plot_index<=N_plot:
                    ...
                    plt.plot(correlation_new,label="t/N="+str(int(time_list_mem[k][j])/N))

            plt.plot(correlation_list_theory[0:len(correlation_new)],c='black',linestyle='--')                 
            plt.ylabel(r"$m_{t,j}$")
            plt.xlabel(r"$\vert t-j\vert/N $")
            plt.title(r'$\alpha=$'+str(α)+r'$\quad\kappa=$'+str(κ)+r'$\quad N=$'+str(N))
            plt.legend()
            plt.show()
            ...
            for j in range(indice_mem):
                ...
                if int(j%20)==1 and plot_index<=N_plot:
                    ...
                    if plot_index==-1:
                        plt.plot(time_list_new/N,correlation_list_theory[0:len(time_list_new)],c='black',linestyle='--',label='theoretical prediction')                     
                    plt.plot(time_list_new/N,correlation_new,label='t/N='+str(int(time_list_mem[k][j])/N))
...
if Monte_Carlo_loop==1:
    ...
    try_size=1
    N_true_list=[40000,3000,7000,10000,20000,40000]
    try_disorder_realization=1
    ...
    if write_mo==1:
        file=open('magnetization_theory_adiabatic_40000(alpha='+str(α)+').txt','w')
        file.write('alpha	kappa_old	kappa_new	N	mag')
        file.write("\n")  

    for i in range(try_size):
        kappa_new_list,m_list=Adiabatic_theory(N_true_list[i],κ_old,α)
        if write_mo==1:
            for j in range(len(kappa_new_list)):
                file.write(str(α))
                file.write('		') 
                file.write(str(κ_old))
                file.write('		') 
                file.write(str(kappa_new_list[j]))
                file.write('		')   
                file.write(str(N_true_list[i]))
                file.write('		')         
                file.write(str(m_list[j]))
                file.write('		')  
                file.write("\n")
    ...
    if write_mo==1:
        file.close()
    if run_MC==1:
        Monte_Carlo(α,κ_old_list,κ_new_max,N_list,write_MC,ticket=-7) 
    
      
#### Quenched MC (remaining frozen , fixed kappa)####
if Monte_Carlo_loop==2:
    ...
    try_size=1
    N_true_list=[1000,3000,7000,10000,20000,40000]
    try_disorder_realization=1
    ...
    if write_mo==1:
        file=open('magnetization_theory_kappa_fixed(alpha='+str(α)+', kappa='+str(κ_new)+').txt','w')
        file.write('alpha	kappa_old	kappa_new	N	mag')
        file.write("\n")  
    ...
    if write_mo==1:
        file.close()
    if run_MC==1:
        Monte_Carlo_2(α,κ_old_list,κ_new_list,N_list,mo_list,write_MC,ticket=-7)
 
    
#### Quenched MC (remaining frozen , fixed mag)####
if Monte_Carlo_loop==21:
    
    mo=0.9
    κ_old=κ_SAT
    
    try_size=1
    N_true_list=[20000,3000,7000,10000,20000,40000]
    ...
    if write_mo==1:
        file=open('magnetization_theory_kappa_not_fixed(alpha='+str(α)+' mag='+str(mo)+').txt','w')
        file.write('alpha	kappa_old	kappa_new	N	mag')
        file.write("\n") 
        
    
    for i in range(try_size):
        κ_new=Quenched_theory_fixed_m(N_true_list[i],mo,κ_old,α)
        if write_mo==1:
            file.write(str(α))
            file.write('		') 
            file.write(str(κ_old))
    ...
    if write_mo==1:
        file.close()
    if run_MC==1:
        Monte_Carlo_2(α,κ_old_list,κ_new_list,N_list,mo_list,write_MC,ticket=-8)
        

#### Quenched MC (into the large web)####
if Monte_Carlo_loop==22:
    ...
    for i in range(try_size):
        ...
        for j in range(try_disorder_realization): 
            ...
            κ_new_list[i*try_disorder_realization+j]=κ_new
            N_list[i*try_disorder_realization+j]    =N_true_list[i]
            mo_list[i*try_disorder_realization+j]   =0

    Monte_Carlo_22(α,κ_old_list,κ_new_list,N_list,mo_list,write=0,ticket=6)
